...
boolean power = false;
SoftwareSerial mySerial(7, 8); //rxtx

String mdata = "Nothing";
  ...
  mdata = "Who are you?";
    ...
    if (incoming_txt=='RI')
    {
      if (incoming_txt=='NG')
      {
              ...
              Serial.println("Hisdaaaaaaaaasdfsfskfsnkvnnskvndskvnksnvksnvksnkdvnksnvkdnskvnk");
      }
     }
...
void SendTextMessage(String &mdata)
{
 mySerial.print("AT+CMGF=1\r"); //Because we want to send the SMS in text mode
 delay(100);
 mySerial.println("AT + CMGS = \"+8801676171200\"");//send sms message, 
 //be careful need to add a country code before the cellphone number
 delay(100);
 mySerial.println(mdata);//the content of the message
 delay(100);
