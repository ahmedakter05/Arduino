...
Temperature::Temperature(OneWire* _oneWire)
  #if REQUIRESALARMS
...
uint8_t Temperature::getDeviceCount(void)
{
  return devices;
}

// returns true if address is valid
bool Temperature::validAddress(uint8_t* deviceAddress)
{
...
bool Temperature::getAddress(uint8_t* deviceAddress, uint8_t index)
{
  uint8_t depth = 0;
...
bool Temperature::isConnected(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
...
bool Temperature::isConnected(uint8_t* deviceAddress, uint8_t* scratchPad)
{
  readScratchPad(deviceAddress, scratchPad);
...
void Temperature::readScratchPad(uint8_t* deviceAddress, uint8_t* scratchPad)
{
  // send the command
  _wire->reset();
...
void Temperature::writeScratchPad(uint8_t* deviceAddress, const uint8_t* scratchPad)
{
  _wire->reset();
  _wire->select(deviceAddress);
...
bool Temperature::readPowerSupply(uint8_t* deviceAddress)
{
  bool ret = false;
  _wire->reset();
  _wire->select(deviceAddress);
...
void Temperature::setResolution(uint8_t newResolution)
{
...
bool Temperature::setResolution(uint8_t* deviceAddress, uint8_t newResolution)
{
  ScratchPad scratchPad;
...
uint8_t Temperature::getResolution()
{
...
uint8_t Temperature::getResolution(uint8_t* deviceAddress)
{
...
void Temperature::setWaitForConversion(bool flag)
{
	waitForConversion = flag;
}

// gets the value of the waitForConversion flag
bool Temperature::getWaitForConversion()
{
...
void Temperature::setCheckForConversion(bool flag)
{
	checkForConversion = flag;
}

// gets the value of the waitForConversion flag
bool Temperature::getCheckForConversion()
{
	return checkForConversion;
}

bool Temperature::isConversionAvailable(uint8_t* deviceAddress)
{
...
bool Temperature::requestTemperaturesByAddress(uint8_t* deviceAddress)
{
...
void Temperature::blockTillConversionComplete(uint8_t* bitResolution, uint8_t* deviceAddress)
{
...
bool Temperature::requestTemperaturesByIndex(uint8_t deviceIndex)
{
  DeviceAddress deviceAddress;
...
float Temperature::getTempCByIndex(uint8_t deviceIndex)
{
  DeviceAddress deviceAddress;
  getAddress(deviceAddress, deviceIndex);
...
// Fetch temperature for device index
float Temperature::getTempFByIndex(uint8_t deviceIndex)
{
  return toFahrenheit(getTempCByIndex(deviceIndex));
}

// reads scratchpad and returns the temperature in degrees C
float Temperature::calculateTemperature(uint8_t* deviceAddress, uint8_t* scratchPad)
{
...
// Temperature.h. It is a large negative number outside the
// operating range of the device
float Temperature::getTempC(uint8_t* deviceAddress)
{
...
float Temperature::getTempF(uint8_t* deviceAddress)
{
  return toFahrenheit(getTempC(deviceAddress));
}

// returns true if the bus requires parasite power
bool Temperature::isParasitePowerMode(void)
{
  return parasite;
}
...
void Temperature::setHighAlarmTemp(uint8_t* deviceAddress, char celsius)
{
...
// accepts a float, but the alarm resolution will ignore anything
// after a decimal point.  valid range is -55C - 125C
void Temperature::setLowAlarmTemp(uint8_t* deviceAddress, char celsius)
{
  // make sure the alarm temperature is within the device's range
  if (celsius > 125) celsius = 125;
  else if (celsius < -55) celsius = -55;
...
char Temperature::getHighAlarmTemp(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
...
// DEVICE_DISCONNECTED for an address
char Temperature::getLowAlarmTemp(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
...
void Temperature::resetAlarmSearch()
{
...
// Temperature::resetAlarmSearch() to start over.
bool Temperature::alarmSearch(uint8_t* newAddr)
{
  uint8_t i;
  char lastJunction = -1;
  uint8_t done = 1;
...
bool Temperature::hasAlarm(uint8_t* deviceAddress)
{
  ScratchPad scratchPad;
  if (isConnected(deviceAddress, scratchPad))
  {
...
bool Temperature::hasAlarm(void)
{
  DeviceAddress deviceAddress;
  resetAlarmSearch();
...
void Temperature::processAlarms(void)
{
  resetAlarmSearch();
...
void Temperature::setAlarmHandler(AlarmHandler *handler)
{
  _AlarmHandler = handler;
}

// The default alarm handler
void Temperature::defaultAlarmHandler(uint8_t* deviceAddress)
{
}
...
float Temperature::toFahrenheit(float celsius)
{
  return (celsius * 1.8) + 32;
}

// Convert float fahrenheit to celsius
float Temperature::toCelsius(float fahrenheit)
{
...
// MnetCS - Allocates memory for Temperature. Allows us to instance a new object
void* Temperature::operator new(unsigned int size) // Implicit NSS obj size
{
  void * p; // void pointer
  p = malloc(size); // Allocate memory
  memset((Temperature*)p,0,size); // Initalise memory

  //!!! CANT EXPLICITLY CALL CONSTRUCTOR - workaround by using an init() methodR - workaround by using an init() method
  return (Temperature*) p; // Cast blank region to NSS pointer
}

// MnetCS 2009 -  Unallocates the memory used by this instance
void Temperature::operator delete(void* p)
{
  Temperature* pNss =  (Temperature*) p; // Cast to NSS pointer
  pNss->~Temperature(); // Destruct the object
